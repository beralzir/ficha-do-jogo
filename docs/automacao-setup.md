...
roda sozinho a cada ~3h: busca placares novos (football-data + ESPN, com quorum), e **só
quando entra jogo novo sem divergência** re-simula, rebuilda, passa no gate e publica no
...
- **Cron:** `17 */3 * * *` (a cada 3h, UTC). Ajuste a cadência no `cron:` se quiser.
...
- **Mata-mata:** detectado, mas **não** é auto-escrito (winner/pênaltis → revisão manual).
